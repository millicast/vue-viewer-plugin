...
exports.createReport = async (req, res) => {
  ...
  const { from, name, email , description, streamId, accountId,
    broadcasterId, serverId, clusterId, url, log } = req.body
  ...
  const broadcaster_id_message =  broadcasterId ?'Broadcaster Token ID: ' + broadcasterId + '\n': ''
  let message_body = 'Stream ID: ' + streamId + '\n Account ID: ' + accountId + '\n Current URL: ' + url + '\n Server ID: ' + serverId 
    + '\n Cluster: ' + clusterId + '\n' + broadcaster_id_message + 'Description: ' + description
  ...
  response = await Intercom.replyConversation(
    conversation_id,
    message
  )
  ...
  response = await Intercom.assignConversation(
    conversation_id,
    {
      "type": "admin",
