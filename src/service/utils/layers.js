...
export const updateLayers = (evntData) => {
    ...
    encodings.forEach(encoding => {
      if (encoding?.active.length === 1 && encoding?.active[0]?.layers.length > 1){
        encoding.active[0]?.layers?.forEach(quality => {
          if (!activeQualities.some(info => info.spatialLayerId === quality.spatialLayerId)) {
        ...
        encoding.inactive[0]?.layers?.forEach(quality => {
          if (!inactiveQualities.some(info => info.spatialLayerId === quality.spatialLayerId)) {
    ...
    commit('Layers/setMedias', { active: activeQualities, inactive: inactiveQualities })
}
...
export const handleSelectQuality = (media) => {
    ...
    const data = (selectedData.encodingId || selectedData.encodingId === 0) ||
    (selectedData.spatialLayerId || selectedData.spatialLayerId === 0) ? selectedData : {}
