...
const tracksAvailableAndMainNotExists = () => {
    return (
        ...
        (!getters['Sources/getAudioHasMain'] &&
            state.Sources.audioSources.length)
    )
}
...
const addSource = (kind, sourceId, trackId) => {
    ...
    const sourceToUse =
        kind === 'video'
            ? state.Sources.videoSources
            : state.Sources.audioSources
...
const deleteSource = (kind, sourceId) => {
    ...
    if (sourceId === selectedSource.sourceId) {
        if (
            sourcesToUse.findIndex((source) => source.sourceId === null) !== -1
        ) {
