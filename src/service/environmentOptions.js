import store from '../store'

export const defaultEnvOptions = {
  NODE_ENV: '',
  VUE_APP_DIRECTOR_ENDPOINT: '',
  VUE_APP_PUBLIC_PATH: '',
  VUE_APP_REPORT_URL: '',
  VUE_APP_CHROMECAST_ID: '',
}

export default function processEnvironmentOptions(environment) {
  const options = {
    NODE_ENV: environment?.NODE_ENV ?? import.meta.env.VITE_DEFAULT_NODE_ENV,
    VUE_APP_DIRECTOR_ENDPOINT:
      environment?.VUE_APP_DIRECTOR_ENDPOINT ??
      import.meta.env.VITE_DEFAULT_DIRECTOR_ENDPOINT,
    VUE_APP_PUBLIC_PATH:
      environment?.VUE_APP_PUBLIC_PATH ??
      import.meta.env.VITE_DEFAULT_PUBLIC_PATH,
    VUE_APP_REPORT_URL:
      environment?.VUE_APP_REPORT_URL ??
      import.meta.env.VITE_DEFAULT_REPORT_URL,
    VUE_APP_CHROMECAST_ID:
      environment?.VUE_APP_CHROMECAST_ID ??
      import.meta.env.VITE_DEFAULT_CHROMECAST_ID,
  }

  store.commit('Params/setEnvironmentOptions', {
    ...defaultEnvOptions,
    ...options,
  })
}
