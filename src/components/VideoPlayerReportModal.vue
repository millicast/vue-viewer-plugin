<template>
 <base-modal :toggle="close">
    ...
    <template v-slot:modal-body>
      <form id="reportForm" @submit.prevent="sendReport">
        <div class="form-group">
          ...
          <input type="text" class="form-control" id="name-input" placeholder="Name Lastname" v-model="report.name" required>
        </div>
        <div class="form-group">
          <label for="email-input">Email</label>
          <input type="email" class="form-control" id="email-input" placeholder="name@example.com" v-model="report.email" required>
        </div>
        <div class="form-group">
          <label for="description-input">Description</label>
          <textarea class="form-control" id="description-input" rows="4" v-model="report.description" required></textarea>
        </div>
      </form>
    </template>
    ...
    <template v-slot:modal-footer>
        ...
        <base-button type="submit" btype="btn-p" form="reportForm" :disabled="isLoading">Submit</base-button>
    </template>
  </base-modal>
</template>
...
export default {
    ...
    methods: {
      async sendReport() {
        ...
        for (let i = 0; i < this.localStats.length; i++) {
          log.push('[PeerConnectionStats] - ' + JSON.stringify(this.localStats[i]))
        }
        ...
        try {
          ...
          this.report.serverId = this.millicastView?.signaling?.serverId ?? 'NOT_CONNECTED'
          this.report.clusterId = this.millicastView?.signaling?.clusterId ?? 'NOT_CONNECTED'
          await fetch(process.env.VUE_APP_REPORT_URL + '/reports', { method: 'POST', headers, body: JSON.stringify(this.report) })
    ...
    mounted() {
      ...
      if (this.millicastView?.webRTCPeer && !this.millicastView?.webRTCPeer?.peerConnectionStats){
