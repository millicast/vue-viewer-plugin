<template>
  <div id="vplayer" ref="player" class="player" :class="{show: show}" @mousemove="showControls">
    <div id="controls" v-if="queryParams.controls">
      ...
      <div class="container-fluid pb-2 gradient-bottom" :class="{show: show, 'fixed-bottom': fullscreen}" style="margin-top: -50px">
        <VideoPlayerControlsContainer :isConnected="cast.isConnected" :showButton="showButton" :currentTime="currentTime" :streamId="queryParams.streamId"/>
      </div>
    </div>

    <div class="overlay d-flex justify-content-center align-items-center" v-if="isLoading">
      <div class="spinner-border text-light" role="status">
        <span class="sr-only">Loading...</span>
      </div>
    </div>

    <div v-if="autoPlayMuted && isLive" @click="tapUnmute" class="overlay tap-unmute d-flex align-items-center justify-content-center">
      <div>
...
export default {
  ...
  mounted () {
    ...
    this.controlsTimeout = setTimeout(() => {this.show = false}, 4000)
  ...
  methods: {
    ...
    ...mapMutations(
      'Controls', ['setVideo','setMobile','setIsLive','setIsLoading','setTrackWarning','setDropup','setVideoVolume','setVideoMuted',
      'setPlaying', 'setCastOptions', 'setAutoPlayMuted']
    ),
    onResize() {
      this.windowWidth = window.innerWidth
    },
