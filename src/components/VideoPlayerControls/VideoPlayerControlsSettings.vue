<template>
  <div class="backdrop" @click="setDropup('')" @dblclick="toggleFullscreen"></div>
  <span class="dropup">
    <i class="ml-viewer-bi-gear-fill h3 align-middle control-icon" @click="setDropup('settings')">
      <span v-if="trackWarning" class="badge bg-light ms-2"> <i class="bi ml-viewer-bi-exclamation-circle-fill p-0"></i></span>
    </i>
    <div ref="settings" class="dropdown-menu dropdown-menu-right" :class="{ show: dropup === 'settings' }">
      <div class="dropdown-header d-flex m-0 col-12">
        <h6 class="p-0 m-0">Settings</h6>
        <div class="p-0 ml-auto" style="color: #9e9e9e;">{{viewerVersion}}</div>
      </div>
      <VideoPlayerControlsSettingsQuality v-if="getActiveMedias.length > 1" />
      <VideoPlayerControlsSettingsVideoTrack v-if="(getVideoSources.length > 1) || (!getVideoHasMain && getVideoSources.length)" :unsupportedFlagEmoji="unsupportedFlagEmoji" :sourceFlagEmojiToPng="sourceFlagEmojiToPng"/>
      <VideoPlayerControlsSettingsAudioTrack v-if="(getAudioSources.length > 1) || (!getAudioHasMain && getAudioSources.length)" :unsupportedFlagEmoji="unsupportedFlagEmoji" :sourceFlagEmojiToPng="sourceFlagEmojiToPng"/>
      <VideoPlayerControlsSettingsStats />
      <VideoPlayerControlsSettingsReportIssue :streamId="streamId"/>
    </div>
    <div class="dropdown-menu dropdown-menu-right" :class="{ show: showDropup }" :style="{ width: settingsWidth }">
      <VideoPlayerControlsSettingsDropdown :selected="selected" :items="items" :compare="compare" :handleClick="handleClick" :title="dropupTitle" :unsupportedFlagEmoji="unsupportedFlagEmoji" :sourceFlagEmojiToPng="sourceFlagEmojiToPng"/>
    </div>
  </span>
</template>
...
export default {
  ...
  computed: {
    ...mapGetters(
      'Layers', ['getActiveMedias']
    ),
    ...mapGetters(
      'Sources', ['getVideoSources', 'getAudioSources', 'getVideoHasMain', 'getAudioHasMain']
    ),
    ...mapState('Layers',{
  ...
  methods: {
    ...
    unsupportedFlagEmoji(sourceId){
      ...
      let isFlagEmoji = sourceId.match(/[\uD83C][\uDDE6-\uDDFF][\uD83C][\uDDE6-\uDDFF]/g) !== null;
      let isWindows
    ...
    sourceFlagEmojiToPng(sourceId){
      let selectedSourceFlagEmojis=sourceId.match(/[\uD83C][\uDDE6-\uDDFF][\uD83C][\uDDE6-\uDDFF]/g);

      // replace emojis  for img
      selectedSourceFlagEmojis.forEach(emoji => {
        
        // get emoji flag code, example france=fr
        let flagCode = Array.from(emoji, (codeUnit) => codeUnit.codePointAt()).map(char => String.fromCharCode(char-127397).toLowerCase()).join('')

        sourceId = sourceId.replace(emoji, ` <img src='https://flagcdn.com/20x15/${flagCode}.png' alt="FlagEmoji"}>`)
  ...
  mounted(){
    this.viewerVersion = process.env.PACKAGE_VERSION ? 'v' + process.env.PACKAGE_VERSION : ''
  },
  watch: {
    dropup: function (dropup) {
      if (dropup === 'videoTracks' || dropup === 'audioTracks' || dropup === 'qualities'){
        ...
        switch (dropup) {
          case 'videoTracks': {
            const videoTrackChange = async (source) => {
              try {
                ...
                toast.error("There was an error selecting the desired source, try again", { timeout: 5000 })
              }
            }
          ...
          case 'qualities': {
            const qualityChange = (media) => { selectQuality(media) }
